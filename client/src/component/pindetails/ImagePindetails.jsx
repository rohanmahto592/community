...
const ImagePindetails = ({user}) => {
  ...
  useEffect(() => {
    const id = pinId;
    axios.get(`http://localhost:3001/posts/images/${id}`).then((response) => {
  ...
  const addComment=async()=>{
    if (file) {
      ...
      const id = pinId;
      axios.put(`http://localhost:3001/posts/images/${id}`,data).then((response)=>{
        setcheck(true)
        setOpen(true)
  ...
  return (
    ...
    <div
        ...
        <Link to={`/user-profile/${doubt?.created_by}`} className="flex gap-2 mt-2 items-center bg-gray-800 rounded-lg ">
            ...
            <Chip  style={{fontFamily:'comic sans ms',color:'slateblue',fontWeight:'bold',marginBottom:'1vh',marginTop:'1vh',}} label={`Article posted ${moment(new Date(doubt?.created_At)).fromNow()}`} />
...
export default ImagePindetails